    ...
    def findFreeRobot(self):                                            # Method for finding a free robot, which could pick up the pallet
        ...
        while robotaddress == '':
            for i in range(0,10):
                ...
                if self.swarmRobotStates[i] == 'Parking':               # the sorting_machine check for a free robot by searching for state "parking" in the list of list of swarm robots states
                    robotId = i + 1
                    if robotId < 10: 
...
def on_message(client, userdata, msg):
    ...
    if msg.topic == "swarm-robot01/status":                                 # Swarm Robots topics, sharing the swarm robot status with the sorting_machine.
