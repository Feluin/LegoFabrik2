...
class Machine:
   ...
   # communicator = None
    ...
    def sort_random(self):
        self.band.moveTo('camera')
        signature=randomSignature()
        self.band.moveTo(signature)  # Move the band to the preset position for the object inside the pallet
        time.sleep(2)
        self.pusher.rotate(signature)  # Pushing the pallet off the band
        self.band.reset(signature)
        return "OK"



def randomSignature():
    return 4 #determined by dice