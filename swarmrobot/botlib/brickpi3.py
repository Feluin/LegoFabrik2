...
class BrickPi3(object):
    ...
    def __init__(self, addr=1, detect=True):
        ...
        if addr < 1 or addr > 255:
            raise IOError(
                "error: SPI address must be in the range of 1 to 255")
            return
        ...
        if detect == True:
            ...
            if vfw.split('.')[0] != FIRMWARE_VERSION_REQUIRED.split('.')[0] or vfw.split('.')[1] != FIRMWARE_VERSION_REQUIRED.split('.')[1]:
                raise FirmwareVersionError("BrickPi3 firmware needs to be version %s but is currently version %s" % (
    ...
    def spi_write_32(self, MessageType, Value):
        ...
        Value -- the value to be sent
        """
        outArray = [self.SPI_Address, MessageType, ((Value >> 24) & 0xFF), ((
            Value >> 16) & 0xFF), ((Value >> 8) & 0xFF), (Value & 0xFF)]
        self.spi_transfer_array(outArray)
    ...
    def get_manufacturer(self):
        ...
        """
        outArray = [self.SPI_Address, self.BPSPI_MESSAGE_TYPE.GET_MANUFACTURER,
    ...
    def get_board(self):
        ...
        """
        outArray = [self.SPI_Address, self.BPSPI_MESSAGE_TYPE.GET_NAME, 0,
    ...
    def get_version_hardware(self):
        ...
        hardware version
        """
        version = self.spi_read_32(
            self.BPSPI_MESSAGE_TYPE.GET_HARDWARE_VERSION)
    ...
    def get_version_firmware(self):
        ...
        """
        version = self.spi_read_32(
            self.BPSPI_MESSAGE_TYPE.GET_FIRMWARE_VERSION)
        return ("%d.%d.%d" % ((version / 1000000), ((version / 1000) % 1000), (version % 1000)))
    ...
    def get_id(self):
        ...
        """
        outArray = [self.SPI_Address, self.BPSPI_MESSAGE_TYPE.GET_ID,
    ...
    def set_sensor_type(self, port, type, params=0):
        ...
        if(type == self.SENSOR_TYPE.CUSTOM):
            outArray = [self.SPI_Address, self.BPSPI_MESSAGE_TYPE.SET_SENSOR_TYPE, int(
                port), type, ((params[0] >> 8) & 0xFF), (params[0] & 0xFF)]

            #self.spi_write_24(self.BPSPI_MESSAGE_TYPE.SET_SENSOR_TYPE, int(port), ((type << 16) + (params[0])))
        elif(type == self.SENSOR_TYPE.I2C):
            if len(params) >= 2:
                outArray = [self.SPI_Address, self.BPSPI_MESSAGE_TYPE.SET_SENSOR_TYPE, int(
                    port), type, params[0], params[1]]  # Settings, SpeedUS
            ...
            outArray = [self.SPI_Address,
                        self.BPSPI_MESSAGE_TYPE.SET_SENSOR_TYPE, int(port), type]
    ...
    def get_sensor(self, port):
        ...
        if self.SensorType[port_index] == self.SENSOR_TYPE.CUSTOM:
            ...
            if(reply[3] == 0xA5):
                if(reply[4] == self.SensorType[port_index] and reply[5] == self.SENSOR_STATE.VALID_DATA):
                    results = [[int(reply[6]), int(reply[7])], [int(reply[8]), int(reply[9])], [
                        int(reply[10]), int(reply[11])], [int(reply[12]), int(reply[13])]]
    ...
    def set_motor_power(self, port, power):
        ...
        """
        outArray = [self.SPI_Address, self.BPSPI_MESSAGE_TYPE.SET_MOTOR_POWER, int(
            port), int(power)]
        self.spi_transfer_array(outArray)
    ...
    def set_motor_position(self, port, position):
        ...
        outArray = [self.SPI_Address, self.BPSPI_MESSAGE_TYPE.SET_MOTOR_POSITION, int(port), ((
    ...
    def set_motor_position_kp(self, port, kp=25):
        ...
        """
        outArray = [self.SPI_Address,
                    self.BPSPI_MESSAGE_TYPE.SET_MOTOR_POSITION_KP, int(port), int(kp)]
        self.spi_transfer_array(outArray)
    ...
    def set_motor_position_kd(self, port, kd=70):
        ...
        """
        outArray = [self.SPI_Address,
                    self.BPSPI_MESSAGE_TYPE.SET_MOTOR_POSITION_KD, int(port), int(kd)]
        self.spi_transfer_array(outArray)
    ...
    def set_motor_dps(self, port, dps):
        ...
        """
        dps = int(dps)
        outArray = [self.SPI_Address, self.BPSPI_MESSAGE_TYPE.SET_MOTOR_DPS, int(
            port), ((dps >> 8) & 0xFF), (dps & 0xFF)]
        self.spi_transfer_array(outArray)
    ...
    def set_motor_limits(self, port, power=0, dps=0):
        ...
        """
        dps = int(dps)
        outArray = [self.SPI_Address, self.BPSPI_MESSAGE_TYPE.SET_MOTOR_LIMITS, int(
            port), int(power), ((dps >> 8) & 0xFF), (dps & 0xFF)]
        self.spi_transfer_array(outArray)
    ...
    def get_motor_status(self, port):
        ...
        if(reply[3] == 0xA5):
            ...
            encoder = int((reply[6] << 24) | (
                reply[7] << 16) | (reply[8] << 8) | reply[9])
    ...
    def offset_motor_encoder(self, port, position):
        ...
        """
        position = int(position)
        outArray = [self.SPI_Address, self.BPSPI_MESSAGE_TYPE.OFFSET_MOTOR_ENCODER, int(port), ((
            position >> 24) & 0xFF), ((position >> 16) & 0xFF), ((position >> 8) & 0xFF), (position & 0xFF)]
        self.spi_transfer_array(outArray)
    ...
    def reset_motor_encoder(self, port):
        ...
        """
        if port & self.PORT_A:
            self.offset_motor_encoder(
                self.PORT_A, self.get_motor_encoder(self.PORT_A))

        if port & self.PORT_B:
            self.offset_motor_encoder(
                self.PORT_B, self.get_motor_encoder(self.PORT_B))

        if port & self.PORT_C:
            self.offset_motor_encoder(
                self.PORT_C, self.get_motor_encoder(self.PORT_C))

        if port & self.PORT_D:
            self.offset_motor_encoder(
                self.PORT_D, self.get_motor_encoder(self.PORT_D))
    ...
    def reset_all(self):
        ...
        """
        # reset all sensors
        self.set_sensor_type(self.PORT_1 + self.PORT_2 +
                             self.PORT_3 + self.PORT_4, self.SENSOR_TYPE.NONE)

        # turn off all motors
        self.set_motor_power(self.PORT_A + self.PORT_B +
                             self.PORT_C + self.PORT_D, self.MOTOR_FLOAT)

        # reset motor limits
        self.set_motor_limits(self.PORT_A + self.PORT_B +
                              self.PORT_C + self.PORT_D)
